...
FLmComponentAndTransform::FLmComponentAndTransform(const FTransform& transform, UPrimitiveComponent* component) {
	Transform = transform;
	Component = component;
}
...
FString FLmDynamicMontageParams::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Animation: %s, PlayRate: %s, StartTime: %s, Blend=[In: %s, Out: %s]" ) ,
	                                *Animation->GetName() ,
	                                *FString::SanitizeFloat( PlayRate ) ,
	                                *FString::SanitizeFloat( StartTime ) ,
	                                *FString::SanitizeFloat( BlendInTime ) ,
	                                *FString::SanitizeFloat( BlendOutTime ) );
...
FLmMantleAsset::FLmMantleAsset(UAnimMontage*  AnimMontage,
                               ...
                               const float    LowPlayRate,
                               const float    LowStartPosition,
                               const float    HighHeight,
                               const float    HighPlayRate,
                               const float    HighStartPosition) {
...
FString FLmMantleAsset::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "AnimMontage: %s, PositionCorrectionCurve: %s,\nStartingOffset: %s,Height: [Low: %s, High: %s], PlayRate: [Low: %s, High: %s], StaringPosition: [Low: %s , High: %s]" ) ,
	                                *AnimMontage->GetName() ,
	                                *PositionCorrectionCurve->GetName() ,
	                                *StartingOffset.ToString() ,
	                                *FString::SanitizeFloat( LowHeight ) ,
	                                *FString::SanitizeFloat( HighHeight ) ,
	                                *FString::SanitizeFloat( LowPlayRate ) ,
	                                *FString::SanitizeFloat( HighPlayRate ) ,
	                                *FString::SanitizeFloat( LowStartPosition ) ,
	                                *FString::SanitizeFloat( HighStartPosition ) );
...
FLmMantleParams::FLmMantleParams(UAnimMontage* animMontage, UCurveVector* positionCorrectionCurve, const float startingPosition, const FVector& startingOffset, const float playRate) {
...
FString FLmMantleParams::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "AnimMontage: %s, PositionCorrectionCurve: %s, StatingOffset: %s, StatingPosition: %s, PlayRate: %s" ) ,
	                                *AnimMontage->GetName() ,
	                                *PositionCorrectionCurve->GetName() ,
	                                *StartingOffset.ToString() ,
	                                *FString::SanitizeFloat( StartingPosition ) ,
	                                *FString::SanitizeFloat( PlayRate ) );
...
FString FLmTurnInPlaceAsset::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Animation: %s, Slot Name: %s, AnimatedAngle: %s, PlayRate: %s, ScaleTurnAngle: %s" ) ,
	                                *Animation->GetName() ,
	                                *SlotName.ToString() ,
	                                *FString::SanitizeFloat( AnimatedAngle ) ,
	                                *FString::SanitizeFloat( PlayRate ) ,
	                                *ULmHelpers::Bool2String( ScaleTurnAngle ) );
...
FLmEssentialValues::FLmEssentialValues(const FVector&  velocity,
                                       ...
                                       const float     speed,
                                       const float     movementInputAmount,
                                       const FRotator& aimingRotation,
                                       const float     aimYawRate) {
	Velocity            = velocity;
...
FString FLmEssentialValues::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Velocity: %ls \nAcceleration: %ls,\nAimingRotation: %ls\nMovementInput: %ls\nSpeed: %f\nMovementInputAmount: %f\nAimYawRate: %f\nIsMoving: %ls\nHasMovementInput: %ls\n" ) ,
	                                ...
	                                *AimingRotation.ToString() ,
	                                *MovementInput.ToString() ,
	                                Speed ,
	                                MovementInputAmount ,
	                                AimYawRate ,
	                                *ULmHelpers::Bool2String( bIsMoving ) ,
	                                *ULmHelpers::Bool2String( bHasMovementInput ) );
...
FLmDebugInfo::FLmDebugInfo(ACharacter* FocusedDebugCharacter,
                           const bool  DebugView,
                           const bool  ShowHUD,
                           const bool  ShowTraces,
                           const bool  ShowDebugShapes,
                           const bool  ShowLayerColors,
                           const bool  SlowMotion,
                           const bool  ShowCharacterInfo) {
...
FString FLmCameraParameters::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "TP FOV: %s, FP FOV: %s, Cam Shoulder? %s" ) ,
	                                *FString::SanitizeFloat( TP_FOV ) ,
	                                *FString::SanitizeFloat( FP_FOV ) ,
	                                *ULmHelpers::Bool2String( bRightShoulder , TEXT( "Right" ) , TEXT( "Left" ) ) );
...
FLmTraceParams::FLmTraceParams(const TEnumAsByte<ETraceTypeQuery> traceChannel, const FVector& traceOrigin, const float traceRadius) {
...
FString FLmRotateInPlaceAsset::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Animation: %s, SlotName: %s, TurnRate: [Slow= %s, Fast= %s], PlayRateRange: [Slow= %s, Fast= %s]" ) ,
	                                *Animation->GetName() ,
	                                *SlotName.ToString() ,
	                                *FString::SanitizeFloat( SlowTurnRate ) ,
	                                *FString::SanitizeFloat( FastTurnRate ) ,
	                                *FString::SanitizeFloat( SlowPlayRate ) ,
	                                *FString::SanitizeFloat( FastPlayRate ) );
...
FString FLmCameraSettings::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "TargetArmLength: %s, SocketOffset: %s, LagSpeed: %s, RotationLagSpeed: %s, DoCollitionTest: %s" ) ,
	                                ...
	                                *SocketOffset.ToString() ,
	                                *FString::SanitizeFloat( LagSpeed ) ,
	                                *FString::SanitizeFloat( RotationLagSpeed ) ,
	                                *ULmHelpers::Bool2String( DoCollisionTest ) );
...
FString FLmMantleTraceSettings::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Ledge Height= [Min: %s ,Max: %s], TraceRadius= [Forward: %s, Downward: %s], Reach Distance: %s" ) ,
	                                *FString::SanitizeFloat( MinLedgeHeight ) ,
	                                *FString::SanitizeFloat( MaxLedgeHeight ) ,
	                                *FString::SanitizeFloat( ForwardTraceRadius ) ,
	                                *FString::SanitizeFloat( DownwardTraceRadius ) ,
	                                *FString::SanitizeFloat( ReachDistance ) );
...
FString FLmMovementSettings::ToString(const bool bPrintToLog) {
	const auto r = FString::Printf( TEXT( "Speeds= [Walk: %s, Run: %s, Sprint: %s], MovementCurve: %s, Rotation RateCurve: %s" ) ,
	                                *FString::SanitizeFloat( WalkSpeed ) ,
	                                *FString::SanitizeFloat( RunSpeed ) ,
	                                *FString::SanitizeFloat( SprintSpeed ) ,
	                                *MovementCurve->GetName() ,
	                                *RotationRateCurve->GetName() );
...
FLmFootLock::FLmFootLock(const FVector& location, const FRotator& rotation, const float alpha) {
	Alpha    = alpha;
	Location = location;
	Rotation = rotation;
}
...
FLmFootOffset::FLmFootOffset(const FVector& target_location, const FVector& offset_location, const FRotator& offset_rotation) {
