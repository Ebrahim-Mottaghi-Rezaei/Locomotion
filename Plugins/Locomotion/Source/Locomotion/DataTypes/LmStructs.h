...
struct FLmMantleAsset {
	...
	FLmMantleAsset();
	FLmMantleAsset(UAnimMontage* AnimMontage, UCurveVector* PositionCorrectionCurve, const FVector& StartingOffset, float LowHeight, float LowPlayRate, float LowStartPosition, float HighHeight, float HighPlayRate, float HighStartPosition);
...
struct FLmEssentialValues {
	...
	FLmEssentialValues();
	FLmEssentialValues(const FVector& velocity, const FVector& acceleration, const FVector& movementInput, bool isMoving, bool hasMovementInput, float speed, float movementInputAmount, const FRotator& aimingRotation, float aimYawRate);
