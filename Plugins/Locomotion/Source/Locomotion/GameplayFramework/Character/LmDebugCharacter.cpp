...
ALmDebugCharacter::ALmDebugCharacter() {
	...
	const ConstructorHelpers::FObjectFinder<USkeletalMesh> MannequinMeshAsset( *MannequinMeshPath );
	if ( MannequinMeshAsset.Succeeded() )
		SkinMesh = MannequinMeshAsset.Object;
	else
		ULmLogger::LogError( "Mannequin Mesh not found." );

	MeshComponent->SetSkeletalMesh( CurrentMeshType == ELmCharacterMeshStyle::Lm_Default ? DefaultMesh : SkinMesh );
