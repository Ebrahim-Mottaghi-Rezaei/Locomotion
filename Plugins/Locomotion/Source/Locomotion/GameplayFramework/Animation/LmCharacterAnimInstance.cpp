...
void ULmCharacterAnimInstance::UpdateFootIK() {
	...
	//Step 1	Updating FootLocking for L/R feet and cache the results
...
void ULmCharacterAnimInstance::DynamicTransitionCheck() {
	...
	const float distanceL = ULmHelpers::GetDistanceBetweenTwoSocketsAndMapRange( GetOwningComponent() ,
	                                                                             FName( TEXT( "ik_foot_l" ) ) ,
	                                                                             RTS_Component ,
	                                                                             FName( TEXT( "VB foot_target_l" ) ) ,
	                                                                             RTS_Component ,
	                                                                             false ,
	                                                                             0.0f ,
	                                                                             0.0f ,
	...
	const float distanceR = ULmHelpers::GetDistanceBetweenTwoSocketsAndMapRange( GetOwningComponent() ,
	                                                                             FName( TEXT( "ik_foot_r" ) ) ,
	                                                                             RTS_Component ,
	                                                                             FName( TEXT( "VB foot_target_r" ) ) ,
	                                                                             RTS_Component ,
	                                                                             false ,
	                                                                             0.0f ,
	                                                                             0.0f ,
...
FLmVelocityBlend ULmCharacterAnimInstance::CalculateVelocityBlend() {
	...
	return FLmVelocityBlend( FMath::Clamp( RelativeDirection.X , 0.0f , 1.0f ) ,
	                         FMath::Abs( FMath::Clamp( RelativeDirection.X , -1.0f , 0.0f ) ) ,
	                         FMath::Abs( FMath::Clamp( RelativeDirection.Y , -1.0f , 0.0f ) ) ,
	                         FMath::Clamp( RelativeDirection.Y , 0.0f , 1.0f ) );
}
...
float ULmCharacterAnimInstance::CalculateLandPrediction() {
	...
	UKismetSystemLibrary::CapsuleTraceSingleByProfile( this ,
	                                                   Start ,
	                                                   End ,
	                                                   ...
	                                                   false ,
	                                                   ToIgnore ,
	                                                   GetDebugTraceType( EDrawDebugTrace::ForOneFrame ) ,
	                                                   HitResult ,
	                                                   true );
...
FLmFootOffset ULmCharacterAnimInstance::SetFootOffsets(FName EnableFootIKCurve, FName IKFootBone, FName RootBone, FLmFootOffset params) {
	...
	if ( GetCurveValue( EnableFootIKCurve ) > 0.0f ) {
		...
		UKismetSystemLibrary::LineTraceSingle( this ,
		                                       IKFootFloorLocation + FVector( 0.0f , 0.0f , IK_TraceDistanceAboveFoot ) ,
		                                       IKFootFloorLocation - FVector( 0.0f , 0.0f , IK_TraceDistanceBelowFoot ) ,
		                                       Channel ,
		                                       false ,
		                                       ToIgnore ,
		                                       GetDebugTraceType( EDrawDebugTrace::ForOneFrame ) ,
		...
		if ( Character->GetCharacterMovement()->IsWalkable( HitResult ) ) {
			...
			TargetRotationOffset = FRotator( -UKismetMathLibrary::DegAtan2( HitResult.ImpactNormal.X , HitResult.ImpactNormal.Z ) ,
			                                 0.0f ,
			                                 UKismetMathLibrary::DegAtan2( HitResult.ImpactNormal.Y , HitResult.ImpactNormal.Z ) );
		}
...
FLmFootLock ULmCharacterAnimInstance::UpdateFootLock(const FName Enable_FootIK_Curve, const FName FootLockCurve, const FName IKFootBone, FLmFootLock lastValue) {
	if ( GetCurveValue( Enable_FootIK_Curve ) > 0.f ) {
		...
		if ( lastValue.Alpha > 0 ) {
			...
			const auto Location_Diff = GetOwningComponent()->GetComponentRotation().UnrotateVector( Velocity * GetWorld()->GetDeltaSeconds() );
...
ELmMovementDirection ULmCharacterAnimInstance::CalculateQuadrant(const ELmMovementDirection Current,
